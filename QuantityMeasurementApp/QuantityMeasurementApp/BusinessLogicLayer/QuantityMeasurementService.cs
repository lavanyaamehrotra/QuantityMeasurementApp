...
namespace QuantityMeasurementApp.BusinessLogicLayer
{
    ...
    public class QuantityMeasurementService : IQuantityMeasurementService
    {
        ...
        /// </summary>
        public Quantity<U> SubtractGenericQuantity<U>(Quantity<U> q1, Quantity<U> q2) where U : class, IMeasurable
        {
            if (q1 == null || q2 == null)
                throw new ArgumentNullException("Quantities cannot be null.");

            Quantity<U> result = q1.Subtract(q2);
            logger.Log($"Subtracted {q1} - {q2} -> {result}");
            return result;
        }
        ...
        /// </summary>
        public Quantity<U> SubtractGenericQuantity<U>(Quantity<U> q1, Quantity<U> q2, U targetUnit) where U : class, IMeasurable
        {
            if (q1 == null || q2 == null)
                throw new ArgumentNullException("Quantities cannot be null.");
            if (targetUnit == null)
            ...
            logger.Log($"Subtracted {q1} - {q2} -> {result} (target: {targetUnit.GetUnitName()})");
            return result;
        }
        ...
        /// </summary>
        public double DivideGenericQuantity<U>(Quantity<U> q1, Quantity<U> q2) where U : class, IMeasurable
        {
            if (q1 == null || q2 == null)
                throw new ArgumentNullException("Quantities cannot be null.");

            double result = q1.Divide(q2);
            logger.Log($"Divided {q1} / {q2} -> {result}");
            return result;
        }
        ...
        /// <summary>
        /// UC14: Creates and validates a temperature Quantity.
        /// Validates: unit not null, value is finite.
        /// Temperature supports equality and conversion only — not arithmetic.
        /// </summary>
        public Quantity<TemperatureUnit> CreateTemperature(double value, TemperatureUnit unit)
        {
            if (unit == null)
                throw new ArgumentException("Temperature unit cannot be null.");
            if (!double.IsFinite(value))
                throw new ArgumentException("Invalid input: Temperature value must be finite (not NaN or Infinity).");

            var temperature = new Quantity<TemperatureUnit>(value, unit);
            logger.Log($"Created temperature: {temperature}");
            return temperature;
        }

        /// <summary>
        /// UC14: Compares two temperature quantities by normalizing both to Kelvin (base unit).
        /// Cross-unit comparison (e.g. 0°C == 32°F) is fully supported.
        /// </summary>
        public bool CompareTemperature(Quantity<TemperatureUnit> t1, Quantity<TemperatureUnit> t2)
        {
            if (t1 == null || t2 == null)
                throw new ArgumentNullException("Temperature quantities cannot be null.");

            bool result = t1.Equals(t2);
            logger.Log($"Compared temperatures: {t1} == {t2} -> {result}");
            return result;
        }

        /// <summary>
        /// UC14: Converts a temperature quantity to the specified target unit.
        /// Uses non-linear conversion formulas (Celsius ↔ Fahrenheit ↔ Kelvin).
        /// </summary>
        public Quantity<TemperatureUnit> ConvertTemperature(Quantity<TemperatureUnit> temperature, TemperatureUnit targetUnit)
        {
            if (temperature == null)
                throw new ArgumentNullException(nameof(temperature), "Temperature quantity cannot be null.");
            if (targetUnit == null)
                throw new ArgumentException("Target temperature unit cannot be null.");

            var result = temperature.ConvertTo(targetUnit);
            logger.Log($"Converted temperature: {temperature} -> {result}");
            return result;
        }
    }
}
